...
Exact-1-in-3-SAT instances reconstructed from the deterministic certificate-tight instance contract.  It uses exact state enumeration throughout.  No sampling
...
Compute the ordinary Adam proposal $d_{\mathrm{A}}$ and keep only its Euclidean
target component,

$$
d=\frac{d_{\mathrm{A}}^{\mathsf{T}}c}{c^{\mathsf{T}}c}c.
$$
