...
MANIFEST = EXPERIMENT / 'instances' / 'manifest.json'
RESULTS = EXPERIMENT / 'results'
RAW = RESULTS / 'raw'
STATES = RESULTS / 'states'
RAW.mkdir(parents=True, exist_ok=True)
STATES.mkdir(parents=True, exist_ok=True)
TRAP = json.loads((EXPERIMENT / 'protocol' / 'trap_definition.json').read_text())['conditions']
...
def main():
    ...
    (RESULTS / 'run_metadata.json').write_text(json.dumps({
        ...
        'implementation_decisions_sha256': __import__('hashlib').sha256((EXPERIMENT / 'protocol' / 'implementation_decisions.md').read_bytes()).hexdigest(),
    }, indent=2) + '\n')
