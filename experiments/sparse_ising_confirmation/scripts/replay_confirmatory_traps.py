...
from qbm_alignment.sparse_ising import Adam,ARMIJO,PARAMETER_SEEDS,PSEUDOINVERSE_CUT,STEPS,build_problem,cosine,effective_spectrum,exact_state,load_manifest,pseudoinverse_direction
BUDGET=1000;TOL=.1
...
def main():
    instances={x.instance_id:x for x in load_manifest(EXPERIMENT / 'instances' / 'manifest.json')};summary=pd.concat([pd.read_csv(p) for p in sorted((EXPERIMENT/'results'/'raw').glob('*_summary.csv'))],ignore_index=True);q=summary[(summary.graph=='full')&(summary.method=='adam')&(summary.initialization=='target_biased')&(summary.boundary_trap==1)];rows=[];logs=[]
    for x in q.itertuples(index=False):
        problem=build_problem(instances[x.instance_id]);r=problem.representations['full'];npz=np.load(EXPERIMENT/'results'/'states'/f'{x.instance_id}_states.npz');theta=npz['full__adam__target_biased__final'][list(PARAMETER_SEEDS).index(int(x.seed))]
    ...
    out=EXPERIMENT/'results'/'replay';out.mkdir(exist_ok=True);pd.DataFrame(rows).to_csv(out/'trap_replay_summary.csv',index=False);pd.DataFrame(logs).to_csv(out/'trap_replay_logs.csv.gz',index=False,compression='gzip');(out/'metadata.json').write_text(json.dumps({'qualifying_traps':len(q),'budget':BUDGET,'methods':['restarted_adam','exact_natural','ray_projection_cooling']},indent=2)+'\n');print('qualifying',len(q),'rows',len(rows))
