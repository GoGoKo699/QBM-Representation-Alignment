...
def main() -> None:
    ...
    # Protocol lock and seed commitment.
    lock = json.loads((ROOT / "PROTOCOL_LOCK.json").read_text(encoding="utf-8"))
    architecture_root = ROOT
    for relative, expected in lock["files"].items():
        actual = hashlib.sha256((architecture_root / relative).read_bytes()).hexdigest()
        if actual != expected:
    ...
    seed_payload = json.loads((ROOT / "CONFIRMATORY_SEED_COMMITMENT.json").read_text(encoding="utf-8"))
    compact_seed = {
    ...
    manifest_path = ROOT / "instances" / "confirmatory_sparse_ising_manifest.json"
    manifest_payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_without_hash = {key: value for key, value in manifest_payload.items() if key != "manifest_sha256"}
    ...
    # Exact spectral and graph checks independent of stored summaries.
    spectral_rows = []
    problems = {}
    for instance in instances:
        problem = build_problem(instance)
        problems[instance.instance_id] = problem
        order = np.argsort(problem.cost, kind="stable")
        ground_index = int(order[0])
        ground_energy = float(problem.cost[ground_index])
        degeneracy = int(np.sum(np.isclose(problem.cost, ground_energy, atol=1e-12, rtol=0.0)))
    ...
    pd.DataFrame(spectral_rows).to_csv(ROOT / "results" / "validation_spectra.csv", index=False)
    checks.append({"name": "24 exact spectra and graph constructions", "passed": True})

    summary_paths = sorted((ROOT / "results" / "raw").glob("*_summary.csv"))
    log_paths = sorted((ROOT / "results" / "raw").glob("*_logs.csv.gz"))
    state_paths = sorted((ROOT / "results" / "states").glob("*_states.npz"))
    ...
    expected_counts = (
        summaries.groupby(["method", "initialization", "graph"]).size().to_dict()
    )
    for method, initialization_count in (("adam", 2), ("exact_natural", 1)):
        for graph in ("chain", "random_tree", "problem_tree", "full"):
            for initialization in (("random", "target_biased") if method == "adam" else ("target_biased",)):
                if expected_counts[(method, initialization, graph)] != 120:
                    raise AssertionError("trajectory-cell count mismatch")

    # Initial and final state checks for all trajectories.
    maximum_initial_residual = 0.0
    maximum_final_gap_residual = 0.0
    maximum_final_pstar_residual = 0.0
    for instance in instances:
        problem = problems[instance.instance_id]
        state_file = np.load(ROOT / "results" / "states" / f"{instance.instance_id}_states.npz")
        ...
        for graph_name, representation in problem.representations.items():
            for initialization in ("random", "target_biased"):
                ...
                for seed_index, seed in enumerate(PARAMETER_SEEDS):
                    expected = canonical_noise(seed, representation.edges)
                    if initialization == "target_biased":
                        expected = expected + representation.coefficients
                    maximum_initial_residual = max(
                        maximum_initial_residual,
                        float(np.max(np.abs(stored_initial[seed_index] - expected))),
                    )
                    row = instance_summary[
                        ...
                        & (instance_summary.seed == seed)
                    ].iloc[0]
                    energy, _gradient, probability, _fisher, _moments = exact_state(stored_final[seed_index], problem, representation, want_fisher=False)
                    gap = energy - instance.ground_energy
                    maximum_final_gap_residual = max(
                        maximum_final_gap_residual, abs(gap - float(row.final_gap))
                    )
                    maximum_final_pstar_residual = max(
                        maximum_final_pstar_residual,
                        abs(float(probability[instance.ground_index]) - float(row.final_pstar)),
                    )
            for seed in PARAMETER_SEEDS:
                initial = state_file[f"{graph_name}__exact_natural__target_biased__seed{seed}__initial"]
                final = state_file[f"{graph_name}__exact_natural__target_biased__seed{seed}__final"]
                expected = representation.coefficients + canonical_noise(seed, representation.edges)
                maximum_initial_residual = max(
                    maximum_initial_residual, float(np.max(np.abs(initial - expected)))
                )
                row = instance_summary[
                    (instance_summary.graph == graph_name)
                ...
                energy, _gradient, probability, _fisher, _moments = exact_state(final, problem, representation, want_fisher=False)
                gap = energy - instance.ground_energy
                maximum_final_gap_residual = max(
                    maximum_final_gap_residual, abs(gap - float(row.final_gap))
                )
    ...
    # Dense log checks: success certificate and exact-natural monotonicity.
    success_rows = []
    ...
    for path in log_paths:
        ...
        for _key, trajectory in exact.groupby(["graph", "seed"]):
            ...
            if difference.size:
                maximum_monotonic_increase = max(maximum_monotonic_increase, float(difference.max()))
                monotonic_violations += int(np.sum(difference > 2e-10))
        for _key, trajectory in frame.groupby(["method", "initialization", "graph", "seed"]):
    ...
    # Aggregate and preregistered decision consistency.
    aggregate = pd.read_csv(ROOT / "results" / "analysis" / "confirmatory_aggregate_summary.csv")
    effects = pd.read_csv(ROOT / "results" / "analysis" / "confirmatory_primary_effects.csv")
    decision = json.loads((ROOT / "results" / "analysis" / "confirmatory_go_no_go.json").read_text())
    if decision["decision"] != "FULL_GO":
        raise AssertionError("preregistered decision is not FULL_GO")
    if not effects.primary_pass.astype(bool).all():
        raise AssertionError("one or more frozen hypotheses failed")
    if not decision["preparation_G4_pass"]:
        raise AssertionError("preparation criterion failed")
    checks.append({"name": "frozen hypothesis and go/no-go decision", "passed": True})

    # Resource-table consistency.
    resources = pd.read_csv(ROOT / "results" / "confirmatory_preparation_resources.csv")
    if len(resources) != 96:
    ...
    validation = {
        "status": "PASS",
        "checks": checks,
        "confirmatory_manifest_sha256": manifest_payload["manifest_sha256"],
        "confirmatory_manifest_file_sha256": hashlib.sha256(manifest_path.read_bytes()).hexdigest(),
