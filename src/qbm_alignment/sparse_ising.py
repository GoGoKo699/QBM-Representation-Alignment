...
def build_problem(instance: InstanceData, *, compute_resources: bool = False) -> Problem:
    ...
    # A state is a one-bit-flip local minimum when no single spin flip lowers
    # the cost.  Vectorizing over all states is equivalent to the original
    # nested loop and makes repeated validation practical.
    states = np.arange(1 << N, dtype=np.int64)
    local = np.ones(1 << N, dtype=bool)
    for mask in FIELD_MASKS:
        local &= cost[states ^ int(mask)] >= cost - 1e-12
    representations: dict[str, Representation] = {}
